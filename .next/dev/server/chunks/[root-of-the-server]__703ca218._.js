...
"[project]/app/api/send-confirmation/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function POST(request) {
    try {
        ...
        const { email, orderNumber, customerName, items, totalPrice, deliveryAddress, deliveryMethod, paymentMethod } = body;
      ...
      Produse comandate:
      ${items.map((item)=>`- ${item.name} x${item.quantity} - ${(item.price * item.quantity).toFixed(2)} RON`).join('\n')}
...
//# sourceMappingURL=%5Broot-of-the-server%5D__703ca218._.js.map